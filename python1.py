rendom=int(input('enetr you number:'))

remainder = rendom % 9
if remainder==0:
    print("The number is divisible by 9")
    print("yes")
    print('We did it')
else:
    print('The number is not divisible by 9 ')
    print('Nooo')
    print('We didnot it')

        


rendom_number=input('enetr you number:')
rendom_number=int(rendom_number)

remainder = rendom % 11
if remainder==0:
    print("The number is divisible by 11")
    print("yes")
    print('We did it')
else:
    print('The number is not divisible by 11 ')
    print('Nooo')
    print('We didnot it')

def check_divisible(divident_value):
    rendom_number=input('enetr you number:')
    rendom_number=int(rendom_number)

    remainder = rendom % divident_value
    if remainder==0:
        print("The number is divisible by", divident_value)
        print("yes")
        print('We did it')
    else:
        print('The number is not divisible by', divident_value )
        print('Nooo')
        print('We didnot it')
            
check_divisible(6)
check_divisible(8)
check_divisible(24)

